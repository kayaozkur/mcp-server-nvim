...
export class SessionHandler {
  ...
  private async createSession(sessionName?: string, autoSave: boolean = true): Promise<string> {
    if (!sessionName) {
  ...
  private async restoreSession(sessionName?: string): Promise<string> {
    if (!sessionName) {
  ...
  private async deleteSession(sessionName?: string): Promise<string> {
    if (!sessionName) {
  ...
  private generateSessionScript(config: any): string {
...
`;
  ...
  async exportSession(sessionName: string, outputPath: string): Promise<string> {
    const sessionFile = path.join(this.sessionDir, `${sessionName}.vim`);
    const metadataFile = path.join(this.sessionDir, `${sessionName}.json`);
  ...
  async importSession(sessionPath: string, newName?: string): Promise<string> {
  ...
  async cloneSession(sourceName: string, targetName: string): Promise<string> {
