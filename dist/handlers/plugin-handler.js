...
export class PluginHandler {
    ...
    async listPlugins(args) {
        ...
        try {
            ...
            if (filter) {
                filteredPlugins = plugins.filter(plugin => plugin.name.toLowerCase().includes(filter.toLowerCase()) ||
        ...
        try {
            ...
            if (await fs.pathExists(this.lazyLockPath)) {
                ...
                for (const [name, info] of Object.entries(lazyLock)) {
                    plugins.push({
                        name,
                        version: info.branch || info.commit || 'latest',
                        source: 'lazy.nvim',
                        status: 'installed',
    ...
    async scanPluginConfigurations() {
        const configs = [];
        try {
            const pluginFiles = [
    ...
    parsePluginConfig(content, file) {
        const plugins = [];
    ...
    getCategoryFromFile(file) {
        ...
        if (file.includes('ui'))
            return 'ui';
        if (file.includes('lsp'))
            return 'language-server';
        if (file.includes('dap'))
            return 'debugging';
        return 'general';
    }
    ...
    async getPluginConfigDetails(pluginName) {
        try {
            const config = {
                name: pluginName,
                found: false
    ...
    extractPluginSection(content, pluginName) {
    ...
    extractKeybindingsForPlugin(content, pluginName) {
        const keybindings = [];
        const lines = content.split('\n');
        let inPluginSection = false;
    ...
    async installPlugin(pluginName, config) {
