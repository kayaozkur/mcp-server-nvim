...
export class OrchestraHandler {
    ...
    async getOrchestraStatus() {
        try {
            ...
            if (await fs.pathExists(this.scriptsDir)) {
                ...
                status.availableScripts = files.filter(file => file.endsWith('.sh') || file.endsWith('.py'));
            }
            return status;
        }
        catch (error) {
    ...
    async manageOrchestraSession(action, sessionName) {
        try {
    ...
    async runScriptSync(scriptPath, args) {
        try {
    ...
    async runScriptAsync(scriptPath, args) {
    ...
    async logOrchestraMessage(message, type, targets) {
        try {
            const messagesFile = path.join(this.orchestraDir, 'messages.log');
    ...
    async updateOrchestraState(action, data) {
        try {
            const stateFile = path.join(this.orchestraDir, 'state.json');
    ...
    async executeClaudeIntegration(command, context) {
        try {
    ...
    async manageConcurrentEditing(action, data) {
        try {
